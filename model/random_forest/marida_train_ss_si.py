...
logging.basicConfig(filename=os.path.join(data_path,'evaluation_rf_ss_si.log'), filemode='a',level=logging.INFO, format='%(name)s - %(levelname)s - %(message)s')
logging.info('*'*10)
...
def main(options):
    ...
    cl_path = os.path.join(data_path, 'rf_classifier_ss_si.joblib')
