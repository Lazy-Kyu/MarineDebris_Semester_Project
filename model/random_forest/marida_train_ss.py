...
logging.basicConfig(filename=os.path.join(data_path,'evaluation_rf_ss.log'), filemode='a',level=logging.INFO, format='%(name)s - %(levelname)s - %(message)s')
logging.info('*'*10)
...
def main(options):
    ...
    df_val = hdf_ss.select('val')
    df_test = hdf_ss.select('test')
    hdf_ss.close()
    ...
    cl_path = os.path.join(data_path, 'rf_classifier_ss.joblib')
