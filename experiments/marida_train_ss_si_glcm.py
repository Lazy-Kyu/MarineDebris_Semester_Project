...
from metrics_marida import confusion_matrix
from assets_marida import conf_mapping, rf_features, cat_mapping_vec
...
logging.basicConfig(filename=os.path.join(data_path,'evaluation_rf_ss_si_glcm.log'), filemode='a',level=logging.INFO, format='%(name)s - %(levelname)s - %(message)s')
logging.info('*'*10)
...
def main(options):
    ...
    cl_path = os.path.join(data_path, 'rf_classifier_ss_si_glcm.joblib')
...
if __name__ == "__main__":
    ...
    parser.add_argument('--gen_masks_path', default=os.path.join(data_path, 'predicted_rf'), help='Path to where to produce store predictions')
